<template>
    ...
    <v-card>
        ...
        <v-card-text>
            ...
            From City : {{city_one}} <br>
            From Province : {{prov_one}} <br>
            To City : {{city_two}} <br>
            To Province : {{prov_two}} <br>
            Travel Date : {{travel_date}} <br>
            Leave Time : {{leave_time}} <br>
         </v-card-text>
<v-card-actions>
    <v-spacer></v-spacer>
    <v-btn class="red darken-3 white--text" @click="dialog = false" >Cancel</v-btn>
    <v-btn class="success" @click="post_ride">Confirm</v-btn>
</v-card-actions>
    </v-card>
